    ...
    <div ref={ref}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className={`card group p-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h3 className="text-xl font-semibold mb-6 text-gray-400">Before Book Shelfie</h3>
        ...
        <div className={`card group p-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '200ms' }}>
          <h3 className="text-xl font-semibold mb-6 text-brand-600">After Book Shelfie</h3>
          <ul className="space-y-4">
            {afterItems.map((item, index) => (
              <li key={index} className={`flex items-center space-x-4 transition-all duration-500 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`} style={{ transitionDelay: `${index * 100 + 200}ms` }}>
